...
export function WorkPackageCard({
  ...
  return (
    <div
      ...
      data-testid={`work-package-card-${child.id}`}
    >
      {!readOnly && (
        <button
      ...
      <div className="min-w-0 flex-1">
        ...
        {child.deliverables && child.deliverables.length > 0 && (
          <div className="mt-1.5 flex flex-wrap gap-1">
            {child.deliverables.map((d, i) => (
              <span
                key={i}
        ...
        {child.dependencies && child.dependencies.length > 0 && (
          <div className="mt-1 flex flex-wrap gap-1">
            {child.dependencies.map((dep, i) => (
              <span
                key={i}
                className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-xs text-blue-700 dark:bg-blue-950/30 dark:text-blue-400"
              >
                {dep}
              </span>
            ))}
