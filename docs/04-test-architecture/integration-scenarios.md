...
### SCN-001: Chat Message → AI Processing → Board Update → WebSocket Broadcast

- **Features involved:** F-2.1, F-2.10, F-2.17, F-3.5, F-3.6, F-6.4
- **Description:** User sends a chat message, AI processes it (after debounce), generates a chat response and board modifications, all changes broadcast to connected clients via WebSocket.
- **Preconditions:** Idea exists with owner connected via WebSocket. AI_MOCK_MODE=True.
- **Steps:**
  ...
  5. AI returns chat response + board node creation instructions
  6. Gateway publishes chat message → WebSocket broadcasts AI message
  7. Core creates board nodes → WebSocket broadcasts `board.node.updated` events
  8. WebSocket broadcasts `ai.processing.completed`
- **Expected result:** Both user and collaborator receive all events in order. Board state reflects AI changes. Chat history includes both user and AI messages.
- **Priority:** P1
- **Layer:** E2E
...
- **Preconditions:** Idea with `chat_message_cap=3` (admin parameter). AI mock mode.
- **Steps:**
...
- **Expected result:** Rate limit enforced per idea. Lockout clears on AI completion. No messages lost.
- **Priority:** P1
- **Layer:** Integration

---

### SCN-003: BRD Generation → Readiness Evaluation → Section Lock → Selective Regeneration

- **Features involved:** F-4.1, F-4.2, F-4.3, F-4.4, F-4.8
- **Description:** User opens Review tab, BRD generates, user locks some sections, then triggers selective regeneration for unlocked sections only.
- **Preconditions:** Idea with sufficient chat history. BRD not yet generated.
- **Steps:**
  1. User opens Review tab → triggers BRD generation
  2. AI generates all 6 sections + readiness evaluation
  3. User edits "Short Description" → section auto-locks
  4. User manually locks "Core Capabilities"
  5. User triggers regeneration with instruction text
  6. AI regenerates only 4 unlocked sections, locked sections preserved
...
### SCN-004: BRD Submit → State Transition → Review Assignment → Notification Chain

- **Features involved:** F-1.5, F-4.5, F-4.7, F-4.10, F-12.1, F-12.5
- **Description:** User submits idea for review, state transitions, BRD version created, reviewers assigned, notifications dispatched.
- **Preconditions:** Idea in "open" state with completed BRD draft. Two reviewers exist.
- **Steps:**
  1. User submits with optional message and selects 2 reviewers
  2. BRD version (v1) created from current draft (immutable snapshot)
  3. PDF generated from BRD version
  4. Idea state transitions: `open` → `in_review`
  5. Review assignments created for both reviewers
  6. Timeline entry created (type: `state_change`)
  7. Brainstorming section locks (read-only)
...
- **Expected result:** State transition atomic. BRD version immutable. Both reviewers notified. Brainstorming locked.
- **Priority:** P1
- **Layer:** Integration
...
- **Description:** Reviewer rejects idea, owner receives notification, reworks BRD, resubmits.
- **Preconditions:** Idea in "in_review" state with reviewer assigned.
- **Steps:**
  1. Reviewer posts timeline comment
  2. Reviewer rejects idea (mandatory comment)
  3. State transitions: `in_review` → `rejected`
  4. Owner notified via persistent notification + email
  5. Brainstorming section unlocks for rework
  6. Owner edits BRD sections
  7. Owner resubmits → state transitions: `rejected` → `in_review`
  8. BRD version v2 created, new PDF generated
  9. Timeline entry created (type: `resubmission`, links v1 and v2)
  10. Reviewers re-notified
- **Expected result:** Full rejection-rework-resubmit cycle works. Both BRD versions accessible. Timeline shows complete audit trail.
- **Priority:** P1
- **Layer:** Integration

---

### SCN-006: Similarity Detection → Merge Flow → New Idea Creation

- **Features involved:** F-5.1, F-5.2, F-5.3, F-5.4, F-5.5, F-5.6, F-5.7
- **Description:** Two ideas develop similar keywords, background matching detects overlap, deep comparison confirms, merge flow initiated and completed.
- **Preconditions:** Two ideas owned by different users, both in "open" state.
- **Steps:**
  1. Keyword Agent generates keywords for both ideas (8+ overlap)
  2. Background matching job detects overlap above threshold (7)
  3. Deep Comparison agent confirms genuine similarity
  4. Both users receive similarity notification (in-app + email)
  5. Both users gain read-only access to each other's idea
  6. User A requests merge → User B's idea locked with merge banner
  7. User B accepts → merge proceeds
  8. New merged idea created with both users as co-owners
  9. Merge Synthesizer generates combined context as first message
  10. AI creates merged board
  11. Original ideas become read-only with reference to new idea
  12. All collaborators from both ideas added to new idea
- **Expected result:** New idea has complete merged state. Original ideas preserved read-only. Both users are co-owners. Merge request marked as completed.
- **Priority:** P1
- **Layer:** E2E

---

### SCN-007: Merge Decline → Permanent Dismissal → Manual Merge Recovery

- **Features involved:** F-5.5, F-5.7, F-5.8
- **Description:** User declines a merge, pair is permanently dismissed, user later manually initiates merge with the same idea.
- **Preconditions:** Merge request exists between two ideas.
- **Steps:**
  1. Target owner declines merge request
  2. Merge request status → `declined`
  3. Target idea unlocks, banner disappears
  4. Background matching never suggests this pair again
  5. User A manually initiates merge request by entering idea UUID
  6. New merge request created for the same pair
  7. Merge proceeds normally on acceptance
- **Expected result:** Automatic suggestion permanently dismissed. Manual merge still possible. No duplicate detection for dismissed pair.
- **Priority:** P2
- **Layer:** Integration

---

### SCN-008: Append Flow (Open → In-Review Idea)

- **Features involved:** F-5.4, F-5.5
- **Description:** Open idea matched to an in-review idea. Owner of open idea requests append. Requires three-way consent.
- **Preconditions:** Idea A (open), Idea B (in_review with reviewer assigned).
- **Steps:**
  1. Similarity detected between A (open) and B (in_review)
  2. Owner A requests append to B
  3. Merge request created with `merge_type=append`
  4. Owner B notified, reviewer of B notified
  5. Owner B accepts (target_owner_consent → accepted)
  6. Reviewer accepts (reviewer_consent → accepted)
  7. Idea A closed (read-only, reference to B)
  8. Owner A becomes collaborator on B
  9. Context from A appended to B
- **Expected result:** Three-way consent enforced. Open idea closed. Owner transferred to collaborator. In-review idea unchanged in review state.
- **Priority:** P1
- **Layer:** Integration

---

### SCN-009: Collaboration Invitation → Accept → Real-Time Join
...
- **Preconditions:** Idea in "open" state. Two users exist.
- **Steps:**
  1. Owner sends collaboration invitation
  2. Invitee notified (bell + email)
  3. Invitation appears on invitee's landing page
  4. Invitee navigates to idea → sees floating banner with accept/decline
  5. Invitee accepts → collaborator record created
  6. Idea visibility changes to "collaborating"
  7. Invitee's WebSocket subscribes to idea's channel group
...
### SCN-010: Context Delegation (Facilitator → Context Agent → Response)
...
- **Preconditions:** Company context populated in context_agent_bucket. RAG chunks indexed.
- **Steps:**
...
### SCN-011: Context Compression Trigger
...
- **Preconditions:** Idea with 50+ chat messages. Context usage approaching threshold (60%).
- **Steps:**
...
### SCN-012: Idea Creation from Landing Page

- **Features involved:** F-9.2, F-2.1, F-2.3, F-2.10
- **Description:** User types first message on landing page, idea created, seamlessly transitions to workspace, AI processes first message.
- **Preconditions:** User authenticated on landing page.
- **Steps:**
  1. User types message in hero section and submits
  2. New idea created via REST API
  3. First chat message created
  4. Browser navigates to `/idea/<uuid>`
  5. WebSocket connection established for new idea
  6. Debounce timer starts
  7. AI processes first message → generates title + initial response
  8. Title displayed and animated
  9. AI response appears in chat
- **Expected result:** Seamless transition. No flicker or reload. Title generated from first message. AI response follows.
- **Priority:** P1
- **Layer:** E2E

---

### SCN-013: Admin Parameter Change → Runtime Effect

- **Features involved:** F-11.3, F-2.10, F-2.11
- **Description:** Admin changes debounce timer parameter, effect applies immediately to active ideas.
- **Preconditions:** Admin user. Active idea with ongoing brainstorming.
- **Steps:**
  1. Admin navigates to Parameters tab
  2. Admin changes `debounce_timer` from 3 to 5 seconds
  3. Parameter saved via REST API
  4. Active idea's next processing cycle uses new 5s debounce
...
### SCN-014: PDF Generation → /TODO Rejection → Manual Fix → Success

- **Features involved:** F-4.4, F-4.7, F-4.9
- **Description:** User enables "Allow Information Gaps", BRD generates with /TODO markers, PDF generation rejected, user fills gaps, PDF generates successfully.
- **Preconditions:** Idea with incomplete chat history. BRD draft exists.
- **Steps:**
  1. User enables "Allow Information Gaps" toggle
  2. BRD regenerates with /TODO markers in 2 sections
  ...
  8. PDF created, BRD version immutable
...
### SCN-015: Soft Delete → Trash → Undo → Permanent Delete

- **Features involved:** F-9.3, F-12.5
- **Description:** User deletes idea, it appears in trash with undo toast, undo restores, re-delete and wait for permanent deletion.
- **Preconditions:** User owns an idea.
- **Steps:**
  1. User deletes idea → `deleted_at` set
  2. Undo toast appears (info type)
  3. Idea moves to Trash list on landing page
  4. User clicks undo within toast duration → `deleted_at` cleared
  5. Idea restored to original list
...
### SCN-016: Board Undo/Redo Across AI and User Actions

- **Features involved:** F-3.7, F-2.17, F-3.2
- **Description:** AI modifies board, user undoes AI action, user makes their own change, redo of AI action unavailable (branched history).
- **Preconditions:** Idea with existing board nodes.
- **Steps:**
  1. AI creates 2 new board nodes + 1 connection
  2. Undo button shows "Undo AI Action"
  3. User clicks undo → AI changes reverted
  4. User creates their own node
...
### SCN-017: Notification Preferences → Selective Email Delivery
...
### SCN-018: Multi-Reviewer Conflict Resolution

- **Features involved:** F-4.11, F-1.5
- **Description:** Two reviewers take conflicting actions on the same idea. Latest action wins.
- **Preconditions:** Idea in "in_review" state with 2 reviewers assigned.
- **Steps:**
  1. Reviewer A starts writing accept comment
  2. Reviewer B rejects idea (with comment) — state → `rejected`
...
### SCN-019: i18n Language Switch → Full Application
...
- **Description:** Complete lifecycle of a user's first idea from creation to acceptance.
...
- **Steps:**
  ...
  3. AI responds with greeting + initial question → Title auto-generated
  4. User and AI exchange 5-6 messages → Board populated with topics
  5. User switches to Board tab → Sees AI-created nodes and connections
  6. User edits a board node → AI modification indicator clears
  7. User opens Review tab → BRD generates on first open
  8. User reviews readiness evaluation → All sections sufficient
  9. User submits with message and reviewer assignment
  10. Reviewer opens Review page → Sees idea in "Assigned to me"
  11. Reviewer reads BRD, posts comment, accepts
  12. Owner receives acceptance notification
  13. Owner visits idea → Everything read-only
...
- **Data cleanup:** Delete created idea and related records.

---

### JOURNEY-002: Collaborative Brainstorming Session

- **Description:** Multi-user real-time collaboration on a single idea.
...
- **Steps:**
  ...
  8. User B edits a different node simultaneously → No conflict
  9. Both users see each other's board changes on commit
- **Assertions at each step:** Presence updates, message delivery, board sync, AI multi-user behavior.
- **Data cleanup:** Delete idea and invitations.

---

### JOURNEY-003: Idea Rejection → Rework → Resubmission → Acceptance
...
- **Data cleanup:** Delete idea and related records.

---

### JOURNEY-004: Merge Detection and Resolution

- **Description:** Two independent ideas converge and get merged.
- **Features covered:** F-5.1, F-5.2, F-5.3, F-5.5, F-5.6
- **Steps:**
  1. User A creates idea about "office space optimization"
  2. User B creates idea about "workplace efficiency improvement"
  3. Both ideas develop through brainstorming → Keywords generated
  4. Background matching detects 8+ keyword overlap
  5. Deep Comparison confirms genuine similarity
...
- **Assertions at each step:** Keywords generated, matching works, notifications sent, merge creates valid new idea.
- **Data cleanup:** Delete all 3 ideas.

---

### JOURNEY-005: Admin Configuration and Monitoring
...
- **Steps:**
  ...
  3. AI Context tab: enters company context in both buckets
  4. Parameters tab: adjusts debounce timer and message cap
  5. Monitoring tab: views active connections, ideas by state
  6. Users tab: searches for a user → sees stats
  7. Returns to brainstorming → New parameters take effect
...
- **Description:** Two users send chat messages at nearly the same time on the same idea.
...
### CONC-002: Board Edit During AI Board Update

- **Description:** User edits a board node while AI is creating/modifying nodes on the same board.
- **Actors:** User, AI (via Board Agent)
- **Steps:**
  1. AI processing started → Board Agent creating nodes
  2. User moves an existing node (drag end → REST PATCH)
  3. AI creates new node (via gRPC → REST)
  4. User's move and AI's creation both succeed
  5. WebSocket broadcasts both changes
  6. Final board state includes user's repositioned node AND AI's new node
- **Expected:** No data corruption. Both changes applied. Last-write-wins for any field-level conflict on same node.
- **Priority:** P1
...
- **Description:** Two reviewers act on the same idea simultaneously.
...
### CONC-004: Merge Request While One User Submits for Review

- **Description:** User A requests merge while User B (target) submits their idea for review.
- **Actors:** User A (merge requester), User B (idea owner submitting)
- **Steps:**
  1. User B clicks "submit for review" at T+0ms
  2. User A clicks "request merge" at T+50ms
  3. If submit processed first: idea transitions to `in_review`, merge request rejected (wrong state for merge, only append possible)
  4. If merge request processed first: idea locked with merge banner, submit fails (idea locked)
- **Expected:** One operation succeeds, other fails gracefully with user-facing error. No inconsistent state.
- **Priority:** P1

---

### CONC-005: Duplicate Collaboration Invitation
...
### CONC-006: AI Processing Restart on New Message
...
- **Expected behavior:**
  ...
  6. Board state unchanged (no partial AI modifications)
...
- **Trigger:** PDF service returns error during BRD version creation.
- **Expected behavior:**
  1. BRD version record created (sections preserved)
  2. `pdf_file_path` remains NULL
  3. Error toast shown to user: "PDF generation failed"
  4. User can retry PDF generation manually
  5. BRD data intact — no data loss
- **Tests:** Assert BRD version exists without PDF. Assert retry works. Assert no data corruption.
- **Priority:** P1
...
- **Trigger:** User's MSAL access token expires during active brainstorming.
- **Expected behavior:**
  ...
  5. After re-login: return to previous idea page
...
- **Trigger:** Summarizing AI generates content not grounded in chat/board data.
- **Expected behavior:**
