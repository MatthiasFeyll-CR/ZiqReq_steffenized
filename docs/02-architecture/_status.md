...
- **Briefing for next specialist:**
  ...
  - **Frontend:** Tailwind CSS 4 + shadcn/ui (Radix primitives), Redux Toolkit (client state — WebSocket, presence), TanStack Query (server state), framer-motion (animations), react-i18next (German + English), Lucide icons, @dnd-kit (drag-and-drop for requirements panel)
  ...
  - **Real-time:** WebSocket via Django Channels. Session-level connection. Requirements panel updates via REST-first + WS broadcast.
