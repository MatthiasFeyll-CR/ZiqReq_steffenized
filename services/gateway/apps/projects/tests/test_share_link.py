...
class TestGenerateShareLink(TestCase):
    """API-SHARE.01: POST /api/projects/:id/share-link — generate token."""
    ...
    def test_generate_share_link_success(self):
        ...
        assert len(token) == 64
        assert data["share_url"] == f"/project/{self.project.id}?token={token}"

        self.project.refresh_from_db()
        assert self.project.share_link_token == token

    def test_regenerate_overwrites_existing_token(self):
        # Generate first token
        resp1 = self.client.post(f"/api/projects/{self.project.id}/share-link")
        token1 = resp1.json()["share_link_token"]

        # Generate second token — should overwrite
        resp2 = self.client.post(f"/api/projects/{self.project.id}/share-link")
        token2 = resp2.json()["share_link_token"]

        assert token1 != token2
        self.project.refresh_from_db()
        assert self.project.share_link_token == token2
...
class TestShareLinkMiddleware(TestCase):
    ...
    def test_valid_token_adds_share_link_viewer_role(self):
        token = "a" * 64
        response = self.client.get(f"/api/projects/{self.project.id}/?token={token}")
        assert response.status_code == 200

    def test_invalid_token_returns_403(self):
        response = self.client.get(f"/api/projects/{self.project.id}/?token=invalidtoken")
        assert response.status_code == 403
        data = response.json()
