...
class TestAdminUsersSearch(TestCase):
    ...
    def test_result_includes_stats_fields(self):
        ...
        for field in ["id", "email", "first_name", "last_name", "display_name", "roles",
                       "idea_count", "review_count", "contribution_count"]:
            assert field in user, f"Missing field: {field}"

    def test_idea_count_owner(self):
        """idea_count counts ideas where user is owner."""
        Idea.objects.create(
            id=uuid.uuid4(), title="Idea 1", owner_id=self.alice.id,
        )
        Idea.objects.create(
            id=uuid.uuid4(), title="Idea 2", owner_id=self.alice.id,
        )
        response = self.client.get("/api/admin/users/search", {"q": "alice"})
        data = response.json()
    ...
    def test_idea_count_excludes_deleted(self):
        ...
        Idea.objects.create(
            id=uuid.uuid4(), title="Deleted Idea", owner_id=self.alice.id,
            deleted_at=timezone.now(),
        )
        response = self.client.get("/api/admin/users/search", {"q": "alice"})
        data = response.json()
        assert data[0]["idea_count"] == 0

    def test_review_count(self):
        """review_count counts review_timeline_entries authored by user."""
        idea = Idea.objects.create(
            id=uuid.uuid4(), title="Idea", owner_id=self.admin.id,
        )
        ReviewTimelineEntry.objects.create(
            id=uuid.uuid4(), idea_id=idea.id, entry_type="comment",
            author_id=self.alice.id, content="Review comment",
        )
        ReviewTimelineEntry.objects.create(
            id=uuid.uuid4(), idea_id=idea.id, entry_type="state_change",
    ...
    def test_contribution_count_chat_messages(self):
        ...
        ChatMessage.objects.create(
            id=uuid.uuid4(), idea_id=idea.id, sender_type="user",
