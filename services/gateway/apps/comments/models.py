...
class ProjectComment(models.Model):
    """A comment or system event entry on a project's comment timeline."""
    ...
    class Meta:
        ...
        ordering = ["created_at"]
        indexes = [
            models.Index(fields=["project_id", "created_at"], name="idx_comments_project_ts"),
        ]

    def __str__(self) -> str:
        if self.is_system_event:
            return f"SystemEvent {self.system_event_type} on project {self.project_id}"
...
class CommentReadStatus(models.Model):
    """Tracks when a user last read the comment section for a project."""
    ...
    def __str__(self) -> str:
        return f"ReadStatus user={self.user_id} project={self.project_id}"
