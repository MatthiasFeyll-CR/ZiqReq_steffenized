...
class TestReviewList(TestCase):
    ...
    def test_ideas_grouped_correctly(self):
        ...
        # Assigned to me should contain the assigned project
        assigned_ids = [i["id"] for i in data["assigned_to_me"]]
        self.assertIn(str(project_assigned.id), assigned_ids)
    ...
    def test_assigned_to_other_reviewer_not_in_assigned_to_me(self):
        ...
        project = Project.objects.create(owner_id=self.owner.id, state="in_review", title="Other Assigned")
        ReviewAssignment.objects.create(
        ...
        assigned_ids = [i["id"] for i in data["assigned_to_me"]]
        self.assertNotIn(str(project.id), assigned_ids)
        ...
        unassigned_ids = [i["id"] for i in data["unassigned"]]
        self.assertNotIn(str(project.id), unassigned_ids)

    def test_unassigned_at_not_null_treated_as_unassigned(self):
        """An project where all assignments have unassigned_at set should appear as unassigned."""
        project = Project.objects.create(owner_id=self.owner.id, state="in_review", title="Was Assigned")
    ...
    def test_response_includes_idea_metadata(self):
        ...
        item = next(i for i in assigned if i["id"] == str(project.id))
        self.assertEqual(item["title"], "Metadata Test")
        self.assertEqual(item["state"], "in_review")
        self.assertEqual(item["owner_name"], "Project Owner")
    ...
    def test_reviewers_list_in_response(self):
        ...
        project = Project.objects.create(owner_id=self.owner.id, state="in_review", title="With Reviewers")
        ReviewAssignment.objects.create(
            project_id=project.id,
            reviewer_id=self.reviewer1.id,
            assigned_by="self",
        ...
        item = next(i for i in data["assigned_to_me"] if i["id"] == str(project.id))
    ...
    def test_empty_review_list(self):
        """No projects returns empty lists for all categories."""
        resp = self.client.get(self._url())
        data = resp.json()
        for category in ["assigned_to_me", "unassigned", "accepted", "rejected", "dropped"]:
